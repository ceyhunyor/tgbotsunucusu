...
<html>
 <head>
  <meta charset="UTF-8">
  <title>BKSP Bot Sunucusu</title>
   <meta name="viewport" content="width=device-width, initial-scale=1.0"> 
</head> 
<body>
<p>Genel Amaçlı Telegram Bot Sunucusu</p>
<p><label for="myText">Telegram JSON:</label>
<input type="text" id="myText"></p>
<p><label for="myText">Telegram URL:</label>
<input type="text" id="tgurl"></p>
<p><label for="myText">Ana Grup:</label>
<input type="text" id="groupid"></p>
...
token = document.getElementById("tgurl").value;
managed = document.getElementById("groupid").value;
...
function sender(value) {
	try {
		...
		if (blacklist.includes(chatid)){
			...
			if(textmes.startsWith("@kopeksorunubot") || textmes.startsWith("/yolla")){
			console.log('Gruptan')
			if(isforward){
			...
			if (textmes.startsWith("/start")){
				fetch(token+'/sendmessage?chat_id='+chatid+'&text=Ho%C5%9Fgeldiniz.%20Platform%20y%C3%B6netimine%20buradan%20mesaj%20yazarak%20ula%C5%9Fabilirsiniz.')
				resultstr += '\n' + chatid+' '+messageid;
				resultjson.data.push(value)
				resultnumber += 1
			} else {
